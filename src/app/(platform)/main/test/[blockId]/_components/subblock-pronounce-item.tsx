...
const SubblockPronounceItem = ({ item }: { item: PronounceItem }) => {
  ...
  const handleSending = async () => {
    ...
    formData.append("word", item?.value);
  ...
  return (
    <div>
      {item?.name && <p className="font-semibold text-xl">{item?.name}</p>}
      <p>
      ...
      {result && !isLoading && (
        <div>
          ...
          <div>
            {result.wordScoreList.map((w: wordScoreItem) => (
              <span
